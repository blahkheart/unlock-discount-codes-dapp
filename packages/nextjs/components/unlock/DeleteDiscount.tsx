...
export const DeleteDiscount: React.FC<Props> = ({ contractName }) => {
  const [lock, setLock] = useState("");
  const [signer, setSigner] = useState("");

  const { writeAsync, isLoading } = useScaffoldContractWrite({
    contractName,
    functionName: "deleteDiscountForLock",
    args: [lock, signer],
      ...
      <div className="flex flex-col w-full mx-5 sm:mx-8 2xl:mx-20">
        <div className="flex flex-col mt-6 px-7 py-8 bg-base-200 opacity-80 rounded-2xl shadow-lg border-2 border-primary">
          <div className="text-center">
            <span className="text-3xl sm:text-3xl text-black">Delete Discount</span>
          </div>
          ...
          <div className="mt-6 w-full">
            ...
            >
              {!isLoading && (
                <>
                  Delete <ArrowSmallRightIcon className="w-3 h-3 mt-0.5" />
                </>
              )}
            </button>
          </div>
